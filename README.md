...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create deck-0c95
  fly apps create deck-0c95-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app deck-0c95
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app deck-0c95-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'deck-0c95-staging' does not match app name in config file 'deck-0c95'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name deck-0c95-db
  fly postgres attach --app deck-0c95 deck-0c95-db

  fly postgres create --name deck-0c95-staging-db
  fly postgres attach --app deck-0c95-staging deck-0c95-staging-db
  ```
